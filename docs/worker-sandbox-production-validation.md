...
```env
JUDGE__SANDBOX_WORKER_POOL_SIZE=32
JUDGE__SANDBOX_STANDARD_PARALLELISM=4
JUDGE__SANDBOX_BORROW_TIMEOUT_SECONDS=0.25
JUDGE__SANDBOX_MAX_QUEUE_WAIT_SECONDS=0.0
JUDGE__SANDBOX_ALLOW_EMERGENCY_WORKER=false
JUDGE__SANDBOX_REQUEST_TIMEOUT_SECONDS=120
JUDGE__SANDBOX_QUEUE_WAIT_WARN_SECONDS=0.5
JUDGE__SANDBOX_HEALTH_CHECK_TIMEOUT_SECONDS=1
```
...
```env
JUDGE__SANDBOX_COMPILATION_CACHE_ENABLED=true
JUDGE__SANDBOX_COMPILATION_CACHE_DIR=/tmp/acoj-ccache
JUDGE__SANDBOX_COMPILATION_CACHE_MAX_MB=512
JUDGE__SANDBOX_COMPILATION_CACHE_TTL_SECONDS=3600
```
...
JUDGE__SANDBOX_ISOLATE_NETWORK=true
JUDGE__SANDBOX_ISOLATE_IPC=true
JUDGE__SANDBOX_ISOLATE_UTS=true
JUDGE__SANDBOX_PRIVATE_MOUNTS=true
JUDGE__SANDBOX_USE_PIVOT_ROOT=true
JUDGE__SANDBOX_BIND_WORKSPACE=true
JUDGE__SANDBOX_ENABLE_CGROUP=true
JUDGE__SANDBOX_CGROUP_VERSION=auto
JUDGE__SANDBOX_CGROUP_BASE_PATH=/sys/fs/cgroup/acoj-sandbox
```

rootfs：

```env
JUDGE__SANDBOX_ROOTFS_PATH=/var/lib/acoj-sandbox/rootfs/cpp
```
...
docker run -d --name acoj-worker-1 \
  ...
  -e JUDGE__SANDBOX_WORKER_POOL_SIZE=32 \
  -e JUDGE__SANDBOX_ENABLE_NAMESPACES=true \
  -e JUDGE__SANDBOX_ENABLE_CGROUP=true \
  acoj-worker \
  python -m celery -A app.platform.tasks.celery_app:celery_app worker \
    -Q judge \
    --pool threads \
